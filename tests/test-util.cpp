...
BOOST_AUTO_TEST_CASE(Basic)
{
  ...
  std::string uncompressed("test");

  for (const auto& s : supported) {;
    BOOST_CHECK_NO_THROW(compress(s, uncompressed.data(), uncompressed.size()));
    auto compressed = compress(s, uncompressed.data(), uncompressed.size());
    BOOST_CHECK_NO_THROW(decompress(s, reinterpret_cast<const char*>(compressed->data()),
                                                                     compressed->size()));
  }
  ...
  for (const auto& s : notSupported) {
    BOOST_CHECK_THROW(compress(s, uncompressed.data(), uncompressed.size()),
                      std::runtime_error);
    // Can't compress here as unsupported
    BOOST_CHECK_THROW(decompress(s, uncompressed.data(), uncompressed.size()),
                      std::runtime_error);
  }
}
